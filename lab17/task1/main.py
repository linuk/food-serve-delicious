...
def index():
    ...
    return render_template('index.html', message=message, page_title=page_title, buttons=buttons, twits=twits,
                           isLogin=isLogin, form=form, url_to_edit_twit=url_to_edit_twit,
...
# APIs
@app.route('/api/v1/twits/<twit_id>')
def api_get_a_twit(twit_id):
    return jsonify({'twits': db.get_a_twit(twit_id)})


@app.route('/api/v1/users/<user_id>')
def api_get_a_user(user_id):
    return jsonify({'user': db.get_user(user_id)})


@app.route('/api/v1/users/<user_id>/twits')
def api_get_twits_of_a_user(user_id):
    return jsonify({'user': db.get_all_twits(user_id)})
