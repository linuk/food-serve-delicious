...
class DBHelper:
    ...
    def add_user(self, username, password):
        ...
        print(query)
        print(self.execute(query, 'commit'))
    ...
    def add_a_new_twit(self, user_id, twit):
        query = 'INSERT INTO twits (twit, user_id, created_at) VALUES ("{}", "{}", "{}")'.format(escape(twit), user_id, datetime.utcnow())
        return self.execute(query, 'commit')

    def update_a_twit(self, twit_id, twit): 
        query = 'UPDATE twits SET twit="{}", created_at="{}" WHERE twit_id={}'.format(escape(twit), datetime.utcnow(), twit_id)
        print(query)
        return self.execute(query, 'commit')
